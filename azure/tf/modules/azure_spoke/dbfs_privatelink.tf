...
resource "azurerm_private_endpoint" "dbfs_dfs" {
  ...
  private_dns_zone_group {
    ...
    private_dns_zone_ids = [azurerm_private_dns_zone.dbfs_dfs[count.index].id] # made change to base code to include [count.index]
  }
...
resource "azurerm_private_dns_zone_virtual_network_link" "dbfs_dfs" {
  ...
  name                  = "dbfs-dfs"
  resource_group_name   = azurerm_resource_group.this.name
  private_dns_zone_name = azurerm_private_dns_zone.dbfs_dfs[count.index].name # made change to base code to include [count.index]
...
resource "azurerm_private_endpoint" "dbfspe_blob" {
  ...
  private_dns_zone_group {
    name                 = "private-dns-zone-dbfs"
    private_dns_zone_ids = [azurerm_private_dns_zone.dbfs_blob[count.index].id] # made change to base code to include [count.index]
  }
...
resource "azurerm_private_dns_zone_virtual_network_link" "dbfs_blob" {
  ...
  name                  = "dbfs-blob"
  resource_group_name   = azurerm_resource_group.this.name
  private_dns_zone_name = azurerm_private_dns_zone.dbfs_blob[count.index].name # made change to base code to include [count.index]
  virtual_network_id    = azurerm_virtual_network.this.id
